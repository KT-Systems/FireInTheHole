...
SPI_HandleTypeDef *hspi         = &BMSCommSPI;       /* MUC SPI Handler      */
//UART_HandleTypeDef *huart       = &huart2;      /* MUC UART Handler     */
//I2C_HandleTypeDef *hi2c         = &hi2c2;       /* MUC I2C Handler      */
...
{
    uint16_t total_size = 4 + size;

    uint8_t tx_buf[total_size];
    uint8_t rx_buf[total_size];

    memcpy(&tx_buf[0], &tx_data[0], 4);

//    for(uint16_t i = 4; i < total_size; i++)
//    {
//        tx_buf[i] = 0xFF;
//    }
    ...
    memcpy(&rx_data[0], &rx_buf[4], size);
}
...
void spiReadBytes(uint16_t size, uint8_t *rx_data)
{
    ...
    HAL_SPI_TransmitReceive(hspi, tx_data, rx_data, size, SPI_TIME_OUT);
}
...
/**
 *******************************************************************************
 * Function: adBmsWakeupIcIdle
 * @brief Wakeup bms ic using chip select from idle
 *
 ...
 *******************************************************************************
*/
void adBmsWakeupIcIdle(uint8_t total_ic)
{
